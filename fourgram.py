...
def train_fourgram_model(train_path: str):
    """Counts unigrams, bigrams, trigrams, and fourgrams efficiently."""
    unigram_counts = Counter()
    bigram_counts = Counter()
    trigram_counts = Counter()
    fourgram_counts = Counter()
    
    print("Training Fourgram Model (Optimized)...")
    ...
    return unigram_counts, bigram_counts, trigram_counts, fourgram_counts, total_unigrams

def predict_fourgram(context: str, candidates: list, 
                     unigram_counts: Counter, bigram_counts: Counter, 
                     trigram_counts: Counter, fourgram_counts: Counter, 
                     total_unigrams: int) -> str:
    if not candidates:
        return ""
    ...
    # Pre-compute context counts OUTSIDE the candidate loop to save millions of dict lookups!
    ...
    # Use standard dict .get() instead of Counter[...] to avoid slow __missing__ calls
...
def main():
    parser = argparse.ArgumentParser(description="Fourgram LM Baseline for Next Word Prediction")
    ...
    parser.add_argument("--output", default="predictions_fourgram.csv", help="Output predictions CSV file")
    parser.add_argument("--model_path", default="fourgram_model.pkl", help="Path to load/save the trained model")
    ...
    if loaded:
        ...
        unigrams, bigrams, trigrams, fourgrams, total_unigrams = train_fourgram_model(args.train)
        print(f"Saving model to {args.model_path}...")
        save_model((vocab, unigrams, bigrams, trigrams, fourgrams, total_unigrams), args.model_path)
    ...
    for context, first_letter in tqdm(zip(contexts, first_letters), total=len(dev_df), desc="Predicting"):
        ...
        pred = predict_fourgram(context, candidates, 
                                unigrams, bigrams, trigrams, fourgrams, total_unigrams)
